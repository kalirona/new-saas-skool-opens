...
from src.services.backups import BackupService, VerifyService
...
backup_cli = typer.Typer(help="Database, config, and storage backup/restore")
cli.add_typer(backup_cli, name="backup")


@backup_cli.command()
def create(
    backup_type: str = typer.Argument("full", help="backup type: full, database, config, storage"),
):
    """Create a backup (full, database, config, or storage)."""
    asyncio.run(_cmd_backup_create(backup_type))


async def _cmd_backup_create(backup_type: str):
    service = BackupService()
    if backup_type == "full":
        results = await service.backup_full()
        for name, result in results.items():
            status = "OK" if result.success else f"FAILED: {result.error}"
            typer.echo(f"  {name}: {result.path} ({status})")
    elif backup_type == "database":
        cfg = get_learnhouse_config()
        db_url = cfg.database.sql_connection_string
        if not db_url:
            typer.echo("ERROR: No database connection string configured", err=True)
            raise typer.Exit(1)
        result = await service.backup_database(db_url)
        typer.echo(f"  database: {result.path} ({'OK' if result.success else 'FAILED'})")
    elif backup_type == "config":
        result = await service.backup_config()
        typer.echo(f"  config: {result.path} ({'OK' if result.success else 'FAILED'})")
    elif backup_type == "storage":
        result = await service.backup_storage()
        typer.echo(f"  storage: {result.path} ({'OK' if result.success else 'FAILED'})")
    else:
        typer.echo(f"Unknown backup type: {backup_type}", err=True)
        raise typer.Exit(1)


@backup_cli.command()
def verify(
    path: str = typer.Argument(..., help="Path to backup file"),
):
    """Verify a backup file (checksum, integrity, dry-run restore)."""
    result = VerifyService.dry_run_restore(path)
    if result.verified:
        typer.echo(f"  Verified: {result.path}")
        typer.echo(f"  Size: {result.size_bytes} bytes")
        typer.echo(f"  Format: {result.format_detected}")
        typer.echo(f"  Integrity: OK")
        if result.details.get("dry_run_success"):
            typer.echo(f"  Dry-run restore: OK ({result.details.get('dry_run_extracted_count', 0)} files)")
    else:
...
@backup_cli.command()
def restore(
    backup_type: str = typer.Argument(..., help="Type: database, storage"),
    path: str = typer.Argument(..., help="Path to backup file"),
):
    """Restore from a backup."""
    asyncio.run(_cmd_backup_restore(backup_type, path))


async def _cmd_backup_restore(backup_type: str, path: str):
    service = BackupService()
    if backup_type == "database":
        ...
        if not db_url:
            typer.echo("ERROR: No database connection string configured", err=True)
            raise typer.Exit(1)
        ok = await service.restore_database(path, db_url)
        typer.echo("  Database restore: OK" if ok else "  Database restore: FAILED")
    elif backup_type == "storage":
        ok = await service.restore_storage(path)
        typer.echo("  Storage restore: OK" if ok else "  Storage restore: FAILED")
    else:
        typer.echo(f"Unknown restore type: {backup_type}", err=True)
        raise typer.Exit(1)

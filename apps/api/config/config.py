...
class HostingConfig(BaseModel):
    ...
    allowed_regexp: str = r"^https?://localhost(:\d+)?$"
    self_hosted: bool
    csp_header: str = ""
...
class InternalPayPalConfig(BaseModel):
...
def get_learnhouse_config() -> LearnHouseConfig:
    ...
    env_region = os.environ.get("LEARNHOUSE_S3_API_REGION")
    env_access_key = os.environ.get("LEARNHOUSE_S3_API_ACCESS_KEY_ID")
    env_secret_key = os.environ.get("LEARNHOUSE_S3_API_SECRET_ACCESS_KEY")
    s3api_yaml = yaml_config.get("hosting_config", {}).get("content_delivery", {}).get("s3api", {})
    bucket_name = s3api_yaml.get("bucket_name") or env_bucket_name
    endpoint_url = s3api_yaml.get("endpoint_url") or env_endpoint_url
    region_name = s3api_yaml.get("region_name") or env_region
    access_key_id = s3api_yaml.get("access_key_id") or env_access_key
    secret_access_key = s3api_yaml.get("secret_access_key") or env_secret_key
    ...
    # PayPal config
    env_paypal_client_id = os.environ.get("LEARNHOUSE_PAYPAL_CLIENT_ID")
    env_paypal_client_secret = os.environ.get("LEARNHOUSE_PAYPAL_CLIENT_SECRET")
    env_paypal_webhook_id = os.environ.get("LEARNHOUSE_PAYPAL_WEBHOOK_ID")

    paypal_client_id = env_paypal_client_id or yaml_config.get("payments_config", {}).get(
        "paypal", {}
    ).get("paypal_client_id")
    paypal_client_secret = env_paypal_client_secret or yaml_config.get("payments_config", {}).get(
        "paypal", {}
    ).get("paypal_client_secret")
    paypal_webhook_id = env_paypal_webhook_id or yaml_config.get("payments_config", {}).get(
        "paypal", {}
    ).get("paypal_webhook_id")
    paypal_sandbox = yaml_config.get("payments_config", {}).get(
        "paypal", {}
    ).get("paypal_sandbox", True)

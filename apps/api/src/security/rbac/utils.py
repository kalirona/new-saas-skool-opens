...
async def check_element_type(element_uuid):
    ...
    if element_uuid.startswith("course_") or element_uuid.startswith("courseupdate_"):
        ...
        return "playgrounds"
    elif element_uuid.startswith("res_"):
        return "resources"
    elif element_uuid.startswith("event_"):
        return "events"
    elif element_uuid.startswith("space_"):
        return "spaces"
    else:
        raise HTTPException(
...
async def get_element_organization_id(
    ...
    if element_type == "courses":
        ...
        return (await db_session.execute(select(Resource.org_id).where(Resource.resource_uuid == element_uuid))).scalars().first()

    elif element_type == "events":
        from src.db.events.events import Event
        return (await db_session.execute(select(Event.org_id).where(Event.event_uuid == element_uuid))).scalars().first()

    elif element_type == "spaces":
        from src.db.communities.spaces import Space
        return (await db_session.execute(select(Space.org_id).where(Space.space_uuid == element_uuid))).scalars().first()

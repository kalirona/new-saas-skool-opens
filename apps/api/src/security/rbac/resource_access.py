...
class ResourceAccessChecker:
    ...
    async def _get_resource(self, resource_uuid: str, config: ResourceConfig):
        ...
        elif config.resource_type == "events":
            from src.db.events.events import Event
            statement = select(Event).where(Event.event_uuid == resource_uuid)
            resource = (await self.db_session.execute(statement)).scalars().first()

        elif config.resource_type == "spaces":
            from src.db.communities.spaces import Space
            statement = select(Space).where(Space.space_uuid == resource_uuid)
            resource = (await self.db_session.execute(statement)).scalars().first()

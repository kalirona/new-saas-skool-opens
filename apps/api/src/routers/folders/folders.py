...
@router.put(
    "/{folder_uuid}/reorder",
    response_model=FolderRead,
    summary="Reorder folder content",
    description="Reorder items in a folder by providing an ordered list of resource UUIDs (drag-and-drop support).",
)
async def api_reorder_folder_content(
    ...
    return await reorder_folder_content(
        request, folder_uuid, resource_uuids, current_user, db_session
    )

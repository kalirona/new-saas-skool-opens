...
async def connect_to_db(app: FastAPI):
    # Enable pgvector extension (outside the schema-check block so the
    # extension-creation commit doesn't interfere with the inspection below).
    ...
    # Schema-authority check — Alembic is the single source of truth,
    # not create_all.  Uses its own connection (outside any transaction).
    if not is_testing:
        from src.core.events.schema_check import verify_schema_version

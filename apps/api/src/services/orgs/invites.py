...
async def get_invite_codes(
    ...
    usergroup_ids = set()
    raw_codes = []
    for invite_code_key in invite_codes:  # type: ignore
        raw = r.get(invite_code_key)
        if raw:
            parsed = json.loads(raw)
            raw_codes.append(parsed)
            if parsed.get("usergroup_id"):
                usergroup_ids.add(parsed["usergroup_id"])

    usergroup_map = {}
    if usergroup_ids:
        usergroups = (await db_session.execute(
            select(UserGroup).where(UserGroup.id.in_(list(usergroup_ids)))
        )).scalars().all()
        usergroup_map = {ug.id: ug.name for ug in usergroups}

    for invite_code in raw_codes:
        invite_code["usergroup_name"] = usergroup_map.get(invite_code.get("usergroup_id"))

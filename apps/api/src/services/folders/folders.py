...
# ----------------------------------------------------------------------------
# Reorder content items within a folder (drag-and-drop support)
# ----------------------------------------------------------------------------
...
async def reorder_folder_content(
    ...
    """Reorder items in a folder by providing an ordered list of resource UUIDs."""
    folder = (
        await db_session.execute(select(Folder).where(Folder.folder_uuid == folder_uuid))
    ).scalars().first()
    if not folder:
    ...
    existing_map = {fc.resource_uuid: fc for fc in existing}

    for idx, res_uuid in enumerate(resource_uuids):
        if res_uuid in existing_map:
            fc = existing_map[res_uuid]
            fc.position = idx
            fc.update_date = str(datetime.now())
            db_session.add(fc)

    await db_session.commit()
    await db_session.refresh(folder)
    return await _folder_to_read(db_session, folder, include_private=True)  # type: ignore[arg-type]

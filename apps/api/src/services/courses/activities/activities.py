...
async def _schedule_embedding(course_id: int, org_id: int) -> None:
    executor = get_executor()
    await executor.run(
        lambda: _trigger_course_embedding(course_id, org_id),
        task_name=f"embed_course_{course_id}",
        idempotency_key=f"embed:{course_id}:{org_id}",
        dlq_payload={"course_id": course_id, "org_id": org_id},
    )
...
async def update_activity(
    ...
    if 'content' in update_data:
        await _schedule_embedding(activity.course_id, activity.org_id)

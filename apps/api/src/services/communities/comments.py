...
async def create_comment(
    ...
    # Create mention and author notifications
    ...
    if discussion.author_id != current_user.id:
        notifications.append(
            NotificationCreate(
                ...
                title=f"{current_user.username} commented on {discussion.title}",
                message=content[:200] if content else None,
                user_id=discussion.author_id,
                org_id=community.org_id,
                actor_id=current_user.id,
                resource_uuid=comment.comment_uuid,
                parent_resource_uuid=discussion.discussion_uuid,
                link=f"/community/{community.community_uuid}/discussion/{discussion.discussion_uuid}",
            )
        )
    ...
    if mentioned_usernames:
        ...
        for mentioned_user_id in mentioned_user_ids:
            if mentioned_user_id != current_user.id and mentioned_user_id != discussion.author_id:
                notifications.append(
                    NotificationCreate(

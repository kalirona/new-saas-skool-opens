...
async def create_discussion(
    ...
    # Notify community members about new discussion
    from src.db.notifications.notifications import NotificationCreate
    from src.services.notifications.notifications import create_notifications_batch
    from src.services.notifications.utils import parse_mentions, resolve_mention_user_ids

    # Find community members to notify (excluding author)
    member_statement = select(CommunityMember.user_id).where(
        CommunityMember.community_id == community.id,
        CommunityMember.user_id != current_user.id,
        CommunityMember.status == "active",
    )
    member_ids = (await db_session.execute(member_statement)).scalars().all()
    ...
    for member_id in member_ids:
        notifications.append(
            NotificationCreate(
                ...
                title=f"New discussion in {community.name}: {discussion.title}",
                message=content[:200] if content else None,
                user_id=member_id,
                org_id=community.org_id,
                actor_id=current_user.id,
                resource_uuid=discussion.discussion_uuid,
                link=f"/community/{community_uuid}/discussion/{discussion.discussion_uuid}",
            )
        )

    # Create mention notifications from title and content
    mentioned_usernames = parse_mentions(f"{title} {content or ''}")
    if mentioned_usernames:
        mentioned_user_ids = await resolve_mention_user_ids(mentioned_usernames, db_session)
        for mentioned_user_id in mentioned_user_ids:
            if mentioned_user_id != current_user.id and mentioned_user_id not in member_ids:
                notifications.append(
                    NotificationCreate(
                        notification_type="mention",
                        title=f"{current_user.username} mentioned you in {discussion.title}",
                        message=content[:200] if content else None,
    ...
    if notifications:
        await create_notifications_batch(notifications, db_session)
...
async def get_discussions_by_community(
    ...
    if space_id is not None:
        query = query.where(Discussion.space_id == space_id)
    ...
    if page == 1:
        ...
        if space_id is not None:
            pinned_query = pinned_query.where(Discussion.space_id == space_id)
    ...
    if space_id is not None:
        non_pinned_query = non_pinned_query.where(Discussion.space_id == space_id)

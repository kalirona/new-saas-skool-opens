...
async def send_reset_password_code(
    ...
    if not user:
        logging.info("Password reset requested for an unknown email")
    ...
    logging.info("Password reset code sent successfully")
    return "If an account with that email exists, a reset code has been sent"
...
async def change_password_with_reset_code(
    ...
    if not user:
        logging.warning("Password change attempted for unknown email")
        raise HTTPException(
            status_code=400,
    ...
    if not reset_code.isalnum():
        logging.warning("Invalid reset code format")
        raise HTTPException(
            status_code=400,
    ...
    if reset_code_value is None:
        logging.warning("Invalid reset code attempt")
        raise HTTPException(
            status_code=400,
            detail="Invalid or expired reset code",
        )
    ...
    if reset_code_object["reset_code_expires"] < int(datetime.now().timestamp()):
        # Delete expired code
        r.delete(reset_key)
        logging.info("Expired reset code used")
        raise HTTPException(
            status_code=400,
            detail="Invalid or expired reset code",
        )

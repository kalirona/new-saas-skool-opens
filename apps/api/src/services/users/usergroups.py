...
async def add_users_to_usergroup(
    ...
    users = (await db_session.execute(
        select(User).where(User.id.in_(user_ids_array))
    )).scalars().all()
    user_map = {u.id: u for u in users}

    existing_uugs = (await db_session.execute(
        select(UserGroupUser).where(
            UserGroupUser.usergroup_id == usergroup_id,
            UserGroupUser.user_id.in_(user_ids_array),
        )
    )).scalars().all()
    existing_uug_set = {(uug.user_id, uug.usergroup_id) for uug in existing_uugs}

    existing_memberships = (await db_session.execute(
        select(UserOrganization).where(
            UserOrganization.org_id == usergroup.org_id,
            UserOrganization.user_id.in_(user_ids_array),
        )
    )).scalars().all()
    membership_set = {m.user_id for m in existing_memberships}

    for user_id in user_ids_array:
        user = user_map.get(user_id)

        if (user_id, usergroup_id) in existing_uug_set:
            logging.error(f"User with id {user_id} already exists in UserGroup")
            continue

        if user_id not in membership_set:
            logging.error(
...
async def remove_users_from_usergroup(
    ...
    for uug in existing_uugs:
        await db_session.delete(uug)
...
async def add_resources_to_usergroup(
    ...
    existing_ugrs = (await db_session.execute(
        select(UserGroupResource).where(
            UserGroupResource.usergroup_id == usergroup_id,
            UserGroupResource.resource_uuid.in_(resources_uuids_array),
        )
    )).scalars().all()
    existing_resource_set = {ugr.resource_uuid for ugr in existing_ugrs}

    for resource_uuid in resources_uuids_array:
        if resource_uuid in existing_resource_set:

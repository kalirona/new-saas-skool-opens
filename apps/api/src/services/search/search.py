...
from src.db.resources.resources import Resource, ResourceRead
...
async def search_across_org(
    ...
    # ── Resources (library resources: pdf, video, audio, link, etc.) ────────
    resources_q = (
        select(Resource)
        .where(Resource.org_id == org.id)
        .where(_ilike_any([Resource.title, Resource.description, Resource.content], pattern))
    )
    if only_public:
        resources_q = resources_q.where(Resource.visibility == "public")
    resources, total_resources = await _paginate_and_count(
        db_session, resources_q, page, limit
    )
    ...
    resource_reads = [ResourceRead.model_validate(r) for r in resources]

...
async def track(
    ...
    await executor.run(
        ...
        task_name=f"analytics_{event_name}",
    )
...
async def _send_event(
    ...
    try:
        ...
        resp = await client.post("/v0/events?name=events", json=payload)
        if resp.status_code >= 500:
            logger.warning("Tinybird ingest server error (%s): %s", resp.status_code, resp.text[:200])
        elif resp.status_code >= 400:
            logger.error("Tinybird ingest client error (%s): %s", resp.status_code, resp.text[:200])
    except Exception:

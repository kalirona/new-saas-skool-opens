...
import { Plus, Search, X, Globe, Lock, Users, ChevronLeft, ChevronRight, Pencil, Trash2, Copy, Download, Box } from 'lucide-react'
    ...
    <FeatureGate feature="playgrounds" orgslug={orgslug} context="dashboard">
        <div className="h-full w-full bg-background pl-4 pr-4 sm:pl-10 sm:pr-10">
          ...
          <div className="mb-6 pt-6">
            <Breadcrumbs
              items={[{ label: 'Playgrounds', href: '/dash/playgrounds', icon: <Box size={14} /> }]}
            />
...
function PlaygroundCard({
  ...
  const accessBadge = {
    public: { icon: <Globe size={12} />, label: 'Public', className: 'bg-green-100 text-green-700' },
    authenticated: { icon: <Users size={12} />, label: 'Members', className: 'bg-blue-100 text-blue-700' },
    restricted: { icon: <Lock size={12} />, label: 'Restricted', className: 'bg-amber-100 text-amber-700' },
  }[playground.access_type as string] ?? { icon: <Lock size={12} />, label: 'Private', className: 'bg-gray-100 text-gray-600' }

'use client'
import { Info, Globe, Users, Image as ImageIcon, Eye, Presentation } from 'lucide-react'
...
function BoardSettingsPage(props: { params: Promise<BoardSettingsParams> }) {
  ...
  if (isLoading || !board) {
    return (
      <div className="h-screen w-full bg-background grid grid-rows-[auto_1fr]">
        <div className="pl-10 pr-10 bg-background nice-shadow animate-pulse">
  ...
  return (
    <div className="h-screen w-full bg-background grid grid-rows-[auto_1fr] grid-cols-1">
      <div className="pl-4 pr-4 sm:pl-10 sm:pr-10 text-sm tracking-tight bg-background z-10 nice-shadow relative min-w-0 overflow-hidden">
        <div className="pt-6 pb-4">
          <Breadcrumbs items={[
            { label: 'Boards', href: '/dash/boards', icon: <Presentation size={14} /> },
            { label: board.name },
          ]} />
        </div>

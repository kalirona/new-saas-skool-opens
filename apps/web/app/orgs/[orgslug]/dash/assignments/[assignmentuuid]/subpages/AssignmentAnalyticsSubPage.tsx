    ...
    return (
        <div className="flex flex-col w-full h-full custom-dots-bg overflow-y-auto">
            <div className="px-10 pt-6 pb-10 flex flex-col space-y-4">
                ...
                <div className="grid grid-cols-4 gap-3">
                    <KpiCard
                        icon={<Crosshair size={14} className="text-indigo-600" />}
                        iconBg="bg-indigo-50"
...
function RankBadge({ rank }: { rank: number }) {
    const palette: Record<number, { bg: string; text: string; icon?: React.ReactNode }> = {
        1: { bg: 'bg-amber-100', text: 'text-amber-700', icon: <Medal size={12} /> },
        2: { bg: 'bg-slate-100', text: 'text-slate-600', icon: <Medal size={12} /> },
        3: { bg: 'bg-orange-100', text: 'text-orange-700', icon: <Medal size={12} /> },
    };

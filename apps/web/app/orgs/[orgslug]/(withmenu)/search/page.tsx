...
import { BookCopy, Folder, Users as UsersIcon, Search as SearchIcon, MessagesSquare, MessageCircle, Mic, ChevronLeft, ChevronRight, Box } from 'lucide-react'
    ...
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 pb-12 max-w-5xl">
        {!urlQuery.trim() ? (
          ...
          <EmptyState
                    icon={<SearchIcon className="w-8 h-8 text-black/40" />}
                    title={t('search.no_results_found')}
                    description={t('search.no_results_description', { query: urlQuery })}
                  />
        ) : (
          <>
    ...
    >
      <div className="relative aspect-video bg-black/5 flex items-center justify-center">
        {imageUrl ? (
          ...
          <FallbackIcon size={24} className="text-black/30" />
        )}
      </div>

'use client'
import { Folder, Search, X, Plus, Upload, FolderPlus } from 'lucide-react'
...
export default function LibraryHeader({
  ...
  return (
    <div className="flex flex-col space-y-2 pt-6">
      <Breadcrumbs items={[{ label: t('library.library'), href: '/dash/library', icon: <Folder size={14} /> }]} />
        ...
        <AuthenticatedClientElement checkMethod="roles" action="create" ressourceType={'folders' as any} orgId={org_id}>
          <div className="flex items-center gap-2">
            ...
            <Modal
              ...
              dialogTrigger={<button className={SECONDARY_BTN}><Upload size={16} /><span>{t('media.upload_media')}</span></button>}
            />
            <Modal
              ...
              dialogTrigger={<button className={PRIMARY_BTN}><FolderPlus size={16} /><span>{t('library.new_folder')}</span></button>}
            />
          </div>

...
import { Link, FileText, Video, Image, Music, File, Play } from 'lucide-react'
...
/*
 ...
 Renders the appropriate rich preview inside the card's aspect-video area:
   ...
   - EMBED Vimeo    -> clean video tile (Video)
...
function ScreenshotPreview({ url, video }: { url: string; video?: boolean }) {
  ...
  if (failed) {
    return <IconTile Icon={video ? Video : Link} />
  }
...
function UploadVideoPreview({ src }: { src: string }) {
  const [failed, setFailed] = React.useState(false)
  if (failed) return <IconTile Icon={Video} />
  return (

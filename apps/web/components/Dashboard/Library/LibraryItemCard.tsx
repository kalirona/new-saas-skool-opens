...
import { MessageCircle, LayoutGrid, Box, Link, FileText, Video, Image, Music, File, MoreVertical, ExternalLink, Download, Lock, Mic, FolderMinus } from 'lucide-react'
        ...
        <div className="pt-1.5 flex items-center justify-between border-t border-gray-100">
          ...
          {href && (
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 inline-flex items-center gap-1">
              ...
              {isUploadMedia ? <Download size={12} /> : <ExternalLink size={12} />}
            </span>
          )}
        </div>
      </div>
            ...
            {href && (
              <DropdownMenuItem asChild>
                <a href={href} target="_blank" rel="noopener noreferrer" className="flex items-center cursor-pointer">
                  {isUploadMedia ? (<><Download className="mr-2 h-4 w-4" /> {t('media.download')}</>) : (<><ExternalLink className="mr-2 h-4 w-4" /> {t('library.open')}</>)}
                </a>
              </DropdownMenuItem>
            )}
            {type === 'media' && (

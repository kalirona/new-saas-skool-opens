...
function CardChip({ brand, last4 }: { brand?: string; last4?: string }) {
  const labels: Record<string, string> = {
    ...
    discover: 'CompactDisc', jcb: 'JCB', unionpay: 'UP',
  }
              ...
              <div key={ch.id} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                <div className="flex items-center space-x-3">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center ${ch.paid ? 'bg-green-100' : 'bg-red-100'}`}>
                    {ch.paid ? <CheckCircle2 size={14} className="text-green-600" /> : <AlertCircle size={14} className="text-red-500" />}
                  </div>
                  <div>
              ...
              <TableRow key={item.enrollment_id}>
                ...
                <TableCell>
                  {offer?.offer_type === 'subscription' ? (
                    <Badge variant="outline" className="flex items-center gap-1 w-fit"><RefreshCcw size={12} /><span>Subscription</span></Badge>
                  ) : (
                    <Badge variant="outline" className="flex items-center gap-1 w-fit"><SquareCheck size={12} /><span>One-time</span></Badge>
                  )}
                </TableCell>

...
function OrgUsers() {
  ...
  const handleBatchRoleChange = async (newRoleUuid: string) => {
    const ids = Array.from(selectedUserIds)
    const toastId = toast.loading(`Updating role for ${ids.length} user(s)...`)
    let successCount = 0
    let errorCount = 0

    for (const userId of ids) {
      try {
        const res = await updateUserRole(org.id, userId, newRoleUuid, access_token)
    ...
    if (errorCount === 0) {
      toast.success(`Role updated for ${successCount} user(s) successfully`)
    } else {
      toast.error(`Updated ${successCount} user(s), ${errorCount} failed`)
    }
            ...
            {selectedUserIds.size > 0 && (
              <div className="flex items-center justify-between px-6 py-3 bg-indigo-50 border-b border-indigo-100">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Select value="" onValueChange={(newRoleUuid) => handleBatchRoleChange(newRoleUuid)}>

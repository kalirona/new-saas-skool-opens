...
import { Radio, UserPlus, GraduationCap, CheckCircle, FileText, Expand, Globe, Smartphone, Monitor, Link } from 'lucide-react'
...
const DEVICE_ICONS: Record<string, React.ReactNode> = {
  ...
  mobile: <Smartphone size={14} className="text-indigo-400" />,
  tablet: <Monitor size={14} className="text-purple-400" />,
  unknown: <Monitor size={14} className="text-gray-400" />,
}
  ...
  return (
    ...
    <div className="bg-white nice-shadow rounded-xl overflow-hidden">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 divide-y sm:divide-y-0 lg:divide-x divide-gray-100">
        ...
        >
          <Expand size={14} className="absolute top-3 right-3 text-gray-300 transition-opacity" />
        ...
        {cards.map((card) => {
          return (
            ...
            >
              {card.modalKey && <Expand size={14} className="absolute top-3 right-3 text-gray-300 transition-opacity" />}
              <div className="flex items-center gap-1.5 mb-1">
                {card.icon}

...
/** MessageSquare bubble display time (ms) */
...
export default function RemoteCursors({ provider, canvasRef, pan, zoom }: RemoteCursorsProps) {
  ...
  // Radio local cursor position — throttled to CURSOR_BROADCAST_INTERVAL
  ...
  // MousePointer DOM elements keyed by clientId. Keeping identity stable prevents
  ...
  useEffect(() => {
    ...
    const paintCursors = () => {
      ...
      // Droplet elements for cursors that are gone

  ...
  return (
    <div className="bg-white rounded-xl nice-shadow overflow-hidden">
      ...
      <div className={`px-4 py-2 flex items-center gap-2 ${isSubscription ? 'bg-indigo-50' : 'bg-gray-50'}`}>
        {isSubscription
          ? <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-indigo-700"><RefreshCcw size={12} /> Subscription</span>
          : <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-600"><SquareCheck size={12} /> One-time payment</span>
        }
      </div>
            ...
            <div className={`space-y-1.5 overflow-hidden transition-all ${expanded ? '' : 'max-h-[3.5rem]'}`}>
              {benefits.map((b, i) => (
                <div key={i} className="flex items-start gap-2 text-xs text-gray-500">
                  <Sparkles size={12} className="text-indigo-300 mt-0.5 shrink-0" />
                  <span>{b}</span>
                </div>
              ))}
            </div>

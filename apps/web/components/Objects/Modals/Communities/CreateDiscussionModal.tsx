...
import { Loader2, AlertCircle, MessageSquare, HelpCircle, Lightbulb, Megaphone, Star, Check, Hash } from 'lucide-react'
...
export function CreateDiscussionModal({
  ...
  orgSlug,
  spaces = [],
  selectedSpaceId: initialSpaceId = null,
  ...
  const [selectedSpaceId, setSelectedSpaceId] = useState<number | null>(
    initialSpaceId ?? (spaces.length > 0 ? spaces[0].id : null)
  )

  // Update default space when spaces array changes (e.g., after first load)
  React.useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (spaces.length > 0 && selectedSpaceId === null) {
      setSpaceError(t('communities.create_discussion.space_required') || 'Please select a space')
      return
    }
    ...
    <Modal
      ...
      onOpenChange={(open) => {
        if (!open) {
          ...
          setSelectedLabel('general')
          setSelectedEmoji(null)
          setSelectedSpaceId(spaces.length > 0 ? spaces[0].id : null)
          onClose()
        }
      }}
          ...
          {spaces.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {t('communities.create_discussion.space_label') || 'Space'} *
              </label>
              <div className="flex flex-wrap gap-2">
                {spaces.map((space) => (
                  <button
                    key={space.id}
                    type="button"
                    onClick={() => { setSelectedSpaceId(space.id); setSpaceError(null) }}
                  ...
                  >
                    <span className="text-sm">{space.icon || <Hash size={16} className="text-gray-400" />}</span>
                    <span className={`text-sm ${selectedSpaceId === space.id ? 'font-medium text-gray-900' : 'text-gray-600'}`}>
                      {space.name}
                    </span>
                    {selectedSpaceId === space.id && (
              ...
              {spaceError && (
                <p className="mt-1 text-sm text-red-500">{spaceError}</p>
              )}
            </div>
          )}
